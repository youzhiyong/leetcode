...
public class EmailUtil {
    ...
    public static void main(String[] args) throws Exception{
        resceive("847371174@qq.com", "epdglurcqwdfbdfi");
    }
    ...
    public static void resceive(String emailAdress, String password) throws Exception {
        ...
        props.setProperty("mail.transport.protocol", port); // 使用的协议（JavaMail规范要求）
        props.setProperty("mail.smtp.host", servicePath); // 发件人的邮箱的 SMTP服务器地址

        // 创建Session实例对象
        Session session = Session.getDefaultInstance(props);
        Store store = session.getStore("pop3");
        store.connect(servicePath, emailAdress, password); //163邮箱程序登录属于第三方登录所以这里的密码是163给的授权密码而并非普通的登录密码
...
//        System.out.println("新邮件: " + folder.getNewMessageCount());
        ...
        System.out.println("新邮件总数: " + folder.getNewMessageCount());
    ...
    public static void parseMessage(Message... messages) throws MessagingException, IOException {
        ...
        for (int i = 0, count = messages.length; i < count; i++) {
            ...
            if (isContainerAttachment) {
                saveAttachment(msg, "H:\\document\\log\\" + msg.getSubject() + "_" + i + "_"); //保存附件
            }
